...
PRIDE_ACCESSIONS = [
        "PXD012460", "PXD012650", "PXD014325", "PXD015297", "PXD016620",
    "PXD016790", "PXD017601", "PXD018855", "PXD018969", "PXD022985",
    "PXD023559", "PXD025729", "PXD025792", "PXD027854", "PXD033012",
    "PXD036036", "PXD036261", "PXD036265", "PXD036770", "PXD037529",
    "PXD039337", "PXD045017", "PXD045471", "PXD045733", "PXD046940",
    "PXD047075", "PXD052254", "PXD052837", "PXD061298", "PXD064519",
]
...
def find_and_download_proteingroups(accession, output_file, session):
    ...
    for record in records:
        name = base(record["fileName"]).casefold()
        if name not in {"proteingroups.txt", "proteingroups.txt.gz"}:
            continue

        try:
            url = download_url(record)
            valid, n_lfq = valid_header(
                read_direct_header(url, session, gz=name.endswith(".gz"))
            )
            if valid:
                candidates.append(("direct", n_lfq, record, url, None))
        except Exception:
            pass

    # 2) proteinGroups.txt inside ZIP archives
    for record in records:
        if not base(record["fileName"]).casefold().endswith(".zip"):
            continue
        ...
        try:
            with RemoteZip(
                ...
                for info in z.infolist():
                    ...
                    if base(member).casefold() not in {"proteingroups.txt", "proteingroups.txt.gz"}:
                        continue
                    valid, n_lfq = valid_header(read_zip_header(z, member))
                    if valid:
                        candidates.append(("remote_zip", n_lfq, record, url, member))
        except Exception:
                ...
                try:
                    ...
                    with ZipFile(local_zip) as z:
                        for info in z.infolist():
                            ...
                            valid, n_lfq = valid_header(read_zip_header(z, member))
                            if valid:
                                # Extract now because temp ZIP disappears.
                                temp_pg = Path(td) / "proteinGroups.txt"
                                extract_zip_member(z, member, temp_pg)
                                candidates.append(
                                    ("local_copy", n_lfq, record, url, str(temp_pg))
                                )

                    # If a valid local copy was found, move best one immediately.
                    local = [c for c in candidates if c[0] == "local_copy"]
                    if local:
                        best = max(local, key=lambda x: x[1])
                        shutil.copy2(best[4], output_file)
                        return
                except Exception:
                    pass

    if not candidates:
        raise FileNotFoundError("No LFQ-containing proteinGroups.txt found.")

    best = max(candidates, key=lambda x: (x[1], x[0] == "direct"))
    kind, _, record, url, member = best

    if kind == "direct":
        with session.get(url, stream=True, timeout=REQUEST_TIMEOUT) as r:
            r.raise_for_status()
            r.raw.decode_content = True
            if base(record["fileName"]).casefold().endswith(".gz"):
        ...
        try:
            with RemoteZip(
                ...
                extract_zip_member(z, member, output_file)
        except Exception:
            with tempfile.TemporaryDirectory() as td:
                local_zip = Path(td) / "archive.zip"
                download_whole(url, local_zip, session)
...
def feature_missingness(df, feature_genes):
    ...
    # any gene appearing in "Gene names" is considered found.
    present = set(
        df["Gene names"]
        .dropna()
        .str.split(";")
...
def main():
    ...
    try:
        for n, accession in enumerate(dict.fromkeys(PRIDE_ACCESSIONS), 1):
            ...
            try:
                raw_file = OUTPUT_DIR / f"{accession}.txt"
                find_and_download_proteingroups(accession, raw_file, session)

                df = create_presence_dataframe(raw_file, session, uniprot_cache)
                dataframes[accession] = df

                if SAVE_PROCESSED_TABLES:
                    df.to_csv(
                        OUTPUT_DIR / f"{accession}_processed.txt",
                        sep="\t",
                        index=False,
                    )
    ...
    dataset_missingness_scores = pd.DataFrame(results)
